...
def __date_features(df: pd.DataFrame):
    ...
    df['weekday_freq'] = df['dayofweek'].apply(lambda x: dayofweek2mean[x])
...
def __price_features(df: pd.DataFrame):
    ...
    eps = 1e-3
    df['log_orders'] = df['total_orders'].apply(np.log1p)
    df['log_discount_amount'] = df['discount_amount'].apply(
        lambda x: np.log(x + eps))
    df['log_discount_price'] = df['discount_price'].apply(
        lambda x: np.log(x + eps))
    df['log_sell_price_main'] = df['sell_price_main'].apply(
        lambda x: np.log(x + eps))
    df['log_total_pred_sales'] = df['total_pred_sales'].apply(
        lambda x: np.log(x + eps))
    df['log_total_pred_discount_sales'] = df['total_pred_discount_sales'].apply(
        lambda x: np.log(x + eps))
    df['log_order_price_ratio'] = df['order_price_ratio'].apply(
        lambda x: np.log(x + eps))
    df['log_order_discount_ratio'] = df['order_discount_ratio'].apply(
        lambda x: np.log(x + eps))
